...
declare namespace firstAid {
    ...
    const TypedArray: typeof TypedArray;
    ...
    const createReadOnlyProxy: <T>(obj: T) => Proxy<T>;
    ...
    const randomFillInsecure: (buffer: BuffferSource) => Uint8Array;
    ...
    const random: (isInsecure?: boolean = false) => number;

    // Returns a standard normal random number.
    // Uses Math.random() if isInsecure is true.
    const randomNormal: (isInsecure?: boolean = false) => number;

    // Returns a standard exponential random number.
    // Uses Math.random() if isInsecure is true.
    const randomExponential: (isInsecure?: boolean = false) => number;

    // Returns a Xenakis random number.
    // Uses Math.random() if isInsecure is true.
    const randomXenakis: (isInsecure?: boolean = false) => number;

    // Returns true in the given probability.
    // Uses Math.random() if isInsecure is true.
    const randomProb: (prob: number, isInsecure?: boolean = false) => boolean;
    ...
    // Uses Math.random() if isInsecure is true.
    const getRandomUuid: (isInsecure?: boolean = false) => string;
